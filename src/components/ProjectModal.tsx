...
export default function ProjectModal({ isOpen, onClose, onSave, editingProject, globalTeam }: ProjectModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white rounded-2xl border border-slate-200 w-full max-w-3xl shadow-xl overflow-hidden my-8 max-h-[90vh] flex flex-col">
        {/* Modal Header */}
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3 border-b border-slate-100 pb-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full text-sm px-3 py-2 border border-slate-300 rounded-lg focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-white"
                />
              </div>
                ...
                <div className="space-y-1.5">
                  {globalTeam && globalTeam.length > 0 && (
                    <select
                      ...
                      className="w-full text-xs px-2 py-1.5 border border-slate-300 rounded-lg focus:outline-hidden focus:ring-2 focus:ring-indigo-500/20 bg-slate-50"
                    >
          ...
          {/* Section 2: Cotation & Priorisation (Required Column) */}
          <div>
            <div className="flex items-center justify-between mb-3 border-b border-slate-100 pb-1">
              ...
              <div className="flex items-center gap-1.5 text-xs text-indigo-700 bg-indigo-50 font-bold px-3 py-1 rounded-full">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-3 bg-slate-50/50 p-3 rounded-xl border border-slate-200/50">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Suivi des Coûts</span>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-0.5">Budget Alloué (€)</label>
                  <input
                    type="number"
                    value={budget}
                    onChange={(e) => setBudget(Number(e.target.value))}
                    className="w-full text-xs px-2 py-1.5 border border-slate-300 rounded-lg focus:outline-hidden bg-white font-mono"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-0.5">Budget Consommé (€)</label>
                  <input
                    type="number"
                    value={spentBudget}
              ...
              {/* Délais */}
              <div className="space-y-3 bg-slate-50/50 p-3 rounded-xl border border-slate-200/50">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Suivi des Délais</span>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-0.5">Date de Début</label>
                  <input
                    type="date"
                    value={startDate}
                    onChange={(e) => setStartDate(e.target.value)}
                    className="w-full text-xs px-2 py-1.5 border border-slate-300 rounded-lg focus:outline-hidden bg-white"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-0.5">Date de Livraison</label>
                  <input
                    type="date"
                    value={endDate}
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-0.5">Alerte Délais</label>
                  <select
                    value={delayLevel}
              ...
              {/* Qualité */}
              <div className="space-y-3 bg-slate-50/50 p-3 rounded-xl border border-slate-200/50">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Suivi de la Qualité</span>
                <div>
                  <div className="flex justify-between mb-0.5">
                    <label className="block text-xs font-medium text-slate-600">Index Qualité</label>
                    <span className="text-xs font-bold text-indigo-600">{qualityIndex}%</span>
                  </div>
                  <input
                    type="range"
                    min="0"
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-600 mb-0.5">Remarques Qualité / Recette</label>
                  <textarea
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-3">
          ...
          <button
            ...
            className="px-5 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 rounded-lg shadow-sm transition-all duration-150"
          >
